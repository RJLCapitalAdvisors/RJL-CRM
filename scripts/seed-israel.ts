...
async function main() {
  ...
  const projects = [
    { name: "Rehavia Gardens", developerId: harel.id, street: "Ramban 12", city: "Jerusalem", neighborhood: "Rehavia", totalUnits: 24, parkingSpaces: 40, stories: 6, completionDate: "06/2027", description: "Boutique new build, two blocks from Gan Sacher." },
    { name: "Ahuza Heights", developerId: benDavid.id, street: "Ahuza 210", city: "Ra'anana", neighborhood: null, totalUnits: 32, parkingSpaces: 58, stories: 8, completionDate: "12/2026", description: "Eight stories on Ahuza with a rooftop penthouse floor." },
  ];
  const projectIds: Record<string, string> = {};
  for (const pr of projects) {
    const found = await prisma.ilProject.findFirst({ where: { name: pr.name } });
    const row = found ? await prisma.ilProject.update({ where: { id: found.id }, data: pr }) : await prisma.ilProject.create({ data: pr });
    projectIds[pr.name] = row.id;
  }
  const apartments = [
    { name: "Rehavia Gardens, Apt 12", street: "Ramban 12", city: "Jerusalem", neighborhood: "Rehavia", projectId: projectIds["Rehavia Gardens"], rooms: 4, completionDate: "06/2027", floor: 3, totalFloors: 6, buildingUnits: 24, internalSqm: 108, mirpesetSqm: 14, ceilingCm: 290, machsanSqm: 6, machsanLocation: "In basement", parkingSpots: "2 side by side", direction: '["South","West"]', mirpesetDirection: '["South"]', mamad: true, priceNis: 5_900_000, developerId: harel.id, agentContactId: noa.id, description: "New build on a quiet street two blocks from Gan Sacher. Delivery with Tofes 4 expected mid 2027." },
