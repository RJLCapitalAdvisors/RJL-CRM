    ...
    <RecordLayout
                ...
                <li key={a.id} className="px-4 py-3 text-sm">
                  ...
                  {(a.contact || a.deal) && (
                    <div className="mt-1 text-xs text-muted">
                      ...
                      {a.deal && <span>· {a.deal.propertyName ?? a.deal.name}</span>}
                    </div>
                  )}
                </li>
              ))}
              {company.activities.length === 0 && <li className="px-4 py-8 text-center text-sm text-muted">No activity logged yet.</li>}
            </ul>
          </div>
        </>
      }
      right={
        <>
          ...
          <AssocCard title="Deals" count={company.deals.length} addHref="/deals/new" empty="Deals where this company is the sponsor will show here.">
            {company.deals.map((d) => (
