...
export default async function HousesPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const where: Prisma.IlHouseWhereInput = {
    AND: [q ? { OR: [{ name: { contains: q, mode: "insensitive" } }, { street: { contains: q, mode: "insensitive" } }, { neighborhood: { contains: q, mode: "insensitive" } }, { city: { contains: q, mode: "insensitive" } }, { developer: { name: { contains: q, mode: "insensitive" } } }] } : {}, city ? { city } : {}],
  };
            ...
            <tbody>
              {rows.map((h) => (
                <tr key={h.id}>
                  ...
                  <td className="text-right tabular-nums">{h.mirpesetSqm ?? ""}</td>
