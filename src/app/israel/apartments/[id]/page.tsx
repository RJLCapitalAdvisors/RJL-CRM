...
export default async function ApartmentPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const [a, developers, people, fx] = await Promise.all([
    prisma.ilApartment.findUnique({
      ...
      select: {
        ...
        parkingSpots: true, direction: true, mirpesetDirection: true, mamad: true, priceNis: true, description: true, floorplanType: true, floorplanName: true, updatedAt: true, developerId: true, agentContactId: true, sellerContactId: true,
      ...
      left={
        <>
          ...
          <AboutCard title="About this apartment">
            <ApartmentForm a={a} fx={fx} action={updateApartment.bind(null, a.id)} autosave />
          </AboutCard>
        </>
      }
      center={
        ...
        <>
          <AssocCard title="Developer" count={a.developer ? 1 : 0} addHref="/israel/companies/new" addLabel="New company" empty="Pick the developer below.">
            ...
            <form action={link} className="flex gap-2 p-3">
              <select name="developerId" defaultValue={a.developerId ?? ""} className="input text-xs">
          ...
          <AssocCard title="Sales agent" count={a.agent ? 1 : 0} addHref="/israel/contacts/new" addLabel="New contact" empty="Pick the agent below (contacts marked Sales agent).">
            ...
            <form action={link} className="flex gap-2 p-3">
              <select name="agentContactId" defaultValue={a.agentContactId ?? ""} className="input text-xs">
          ...
          <AssocCard title="Seller" count={a.seller ? 1 : 0} empty="Pick the seller below (contacts marked Seller).">
            ...
            <form action={link} className="flex gap-2 p-3">
              <select name="sellerContactId" defaultValue={a.sellerContactId ?? ""} className="input text-xs">
