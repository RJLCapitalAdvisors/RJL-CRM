...
export async function createDealFromIntake(id: string): Promise<string> {
  ...
  const deal = await prisma.deal.create({
    data: {
      ...
      projectedSellout: d.projectedSellout,
      selloutPerUnit: d.selloutPerUnit ?? (d.projectedSellout && d.units ? Math.round(d.projectedSellout / d.units) : null),
      selloutPerFoot: d.selloutPerFoot ?? (d.projectedSellout && d.squareFeet ? Math.round((d.projectedSellout / d.squareFeet) * 100) / 100 : null),
