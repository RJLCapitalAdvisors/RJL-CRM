...
export default async function NewCampaignPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const [deals, templates, deal] = await Promise.all([
    ...
    prisma.emailTemplate.findMany({ orderBy: { updatedAt: "desc" }, select: { id: true, name: true } }),
    dealId ? prisma.deal.findUnique({ where: { id: dealId } }) : null,
  ]);
  const audience = deal ? await buildAudience(deal, role || null) : [];
  ...
  return (
    <>
      <PageHeader title="New campaign" subtitle="Pick a deal and a template, then choose who receives it" />
      <div className="space-y-6 px-8 py-6">
        <form method="get" className="card grid grid-cols-5 gap-4 p-5">
        ...
        {!deal && <Empty>Select a deal to build the audience. Active deals only; the template can be picked after.</Empty>}
            ...
            <form action={createCampaign} className="space-y-4">
              ...
              <div className="card grid grid-cols-3 gap-4 p-5">
                <div>
                  ...
                  <input id="name" name="name" className="input" defaultValue={`${deal.propertyName ?? deal.name} – ${role || "All"}s`} />
                </div>
                <div>
