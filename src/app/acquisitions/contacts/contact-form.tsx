...
type Ct = Partial<{
  ...
  pipelineAt: Date | string | null;
  pipelinePriority: number | null;
  ...
  const body = (
    <>
      ...
      <Group title="Role">
        ...
        {c.id && (
          <Row label="Pipeline" hint="Track this person on the Buyers or Operators pipeline list as well (by role); they stay here too. Priority 1 to 5 orders the list.">
            <div className="py-1">
              <PipelineToggle kind="contact" id={c.id} at={c.pipelineAt} priority={c.pipelinePriority} />
            </div>
          </Row>
        )}
      </Group>
