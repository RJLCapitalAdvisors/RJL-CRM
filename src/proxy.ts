 ...
 */
const PUBLIC = [/^\/login/, /^\/api\/auth\//, /^\/api\/graph\//, /^\/api\/cron\//, /^\/share\//, /^\/unsubscribe\//, /^\/api\/inbound/, /^\/api\/deals\/[^/]+\/(files\/|faq\.pdf|progress-report\.pdf)/, /^\/logo\.png$/, /^\/outlook-bridge\//, /^\/favicon/, /^\/_next\//];
