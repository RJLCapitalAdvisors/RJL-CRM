 ...
 */
const PUBLIC = [/^\/login/, /^\/api\/auth\//, /^\/api\/graph\//, /^\/api\/cron\//, /^\/share\//, /^\/unsubscribe\//, /^\/api\/inbound/, /^\/api\/deals\/[^/]+\/(files\/|faq\.pdf|progress-report\.pdf)/, /^\/logo\.png$/, /^\/israel-logo\.(png|svg)$/, /^\/acquisitions-logo\.svg$/, /^\/[^/]+\.(svg|png|ico|webp|jpg)$/, /^\/outlook-bridge\//, /^\/favicon/, /^\/_next\//, ...(process.env.NODE_ENV === "development" ? [/^\/dev\//] : [])]; // /dev pages exist only on a developer machine
