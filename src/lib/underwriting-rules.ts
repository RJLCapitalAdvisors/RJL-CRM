...
export const DEFAULT_UNDERWRITING_RULES: string[] = [
  ...
  "Condo (for-sale) developments: assetClass is Condo, strategy Development. There is no NOI, so leave yieldOnCost, cashOnCash and both cap rates blank. Fill projectedSellout (the total projected gross sellout of every unit, from the model's sales or revenue schedule), selloutPerUnit (average sale price per unit) and selloutPerFoot (sale price per sellable square foot); units and squareFeet are the sellable units and sellable area. A pref request on a condo is read like any pref: requestedAmount, total debt, total capitalization and the last dollar exposure; the stabilized basis on the last pref dollar is quoted per foot.",
