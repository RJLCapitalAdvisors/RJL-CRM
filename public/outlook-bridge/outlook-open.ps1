...
if ($mode -eq 'test') {
  ...
  $who = ''; try { $who = $ns.CurrentUser.Name } catch {}
  [System.Windows.Forms.MessageBox]::Show("RJL CRM is connected to Outlook on this computer." + $(if ($who) { " Outlook profile: $who." } else { "" }) + "`n`nHandle and Open in Outlook buttons will open drafts here.", 'RJL CRM') | Out-Null
  Log 'test ok'
  exit 0
}
